...
implements TextListener, FocusListener {
    ...
    public void setJMthisInterfacedioCfg ( JMAppsCfg cfgJMApps ) {
        int                     i;
    ...
    private void init () throws Exception {
    	...
    	panelDescription.add ( new Label(JMFI18N.getResource("jmthisInterfacedio.export.targetnw.label1")) );
    	panelDescription.add ( new Label(JMFI18N.getResource("jmthisInterfacedio.export.targetnw.label2")) );
    ...
    public void setTracks ( boolean arrVideoTracksEnabled[], boolean arrAudioTracksEnabled[] ) {
        ...
        String  strAudio = JMFI18N.getResource("jmthisInterfacedio.export.targetnw.audio");
        String  strVideo = JMFI18N.getResource("jmthisInterfacedio.export.targetnw.video");
    	...
    	label = new Label ( JMFI18N.getResource("jmthisInterfacedio.export.targetnw.track"), Label.CENTER );
    	panelLabels.add ( label );
    	label = new Label ( JMFI18N.getResource("jmthisInterfacedio.export.targetnw.sessionaddr"), Label.CENTER );
    	panelAddress.add ( label );
    	label = new Label ( JMFI18N.getResource("jmthisInterfacedio.export.targetnw.port"), Label.CENTER );
    	panelPort.add ( label );
    	label = new Label ( JMFI18N.getResource("jmthisInterfacedio.export.targetnw.ttl"), Label.CENTER );
    	panelTtl.add ( label );
    ...
    public boolean checkValidFields ( boolean boolDisplayMessage ) {
        ...
        for ( i = 0;  i < nVideoCount;  i++ ) {
                ...
                if ( nValue < 0  ||  nValue > 255 ) {
                    if ( boolDisplayMessage == true )
                        MessageDialog.createErrorDialog ( getFrame(),
                                        JMFI18N.getResource("jmthisInterfacedio.error.video.sessionaddr")
                                        + " " + (i + 1) );
                    return ( false );
                }
            }
            ...
            if ( nValue < 0  ||  (nValue & 0x0001) == 0x0001 ) {
                if ( boolDisplayMessage == true )
                    MessageDialog.createErrorDialog ( getFrame(),
                                JMFI18N.getResource("jmthisInterfacedio.error.video.port")
                                + " " + (i + 1) );
                return ( false );
            }
        ...
        for ( i = 0;  i < nAudioCount;  i++ ) {
            for ( j = 0;  j < 4;  j++ ) {
                ...
                if ( nValue < 0  ||  nValue > 255 ) {
                    if ( boolDisplayMessage == true )
                        MessageDialog.createErrorDialog ( getFrame(),
                                        JMFI18N.getResource("jmthisInterfacedio.error.audio.sessionaddr")
                                        + " " + (i + 1) );
                    return ( false );
                }
            }
            ...
            if ( nValue < 0  ||  (nValue & 0x0001) == 0x0001 ) {
                if ( boolDisplayMessage == true )
                    MessageDialog.createErrorDialog ( getFrame(),
                                JMFI18N.getResource("jmthisInterfacedio.error.audio.port")
                                + " " + (i + 1) );
                return ( false );
            }
